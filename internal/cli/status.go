...
func runStatus(cmd *cobra.Command, args []string) error {
	...
	if s.SchemaVersion == "" && len(s.ActiveAgents) == 0 {
		fmt.Fprintln(out, "No installation found.")
		fmt.Fprintf(out, "Run 'devrune resolve' then 'devrune install' to get started.\n")
		return nil
	}
	...
	fmt.Fprintf(out, "Lock hash:       %s\n", s.LockHash)

	if len(s.ActiveAgents) > 0 {
		fmt.Fprintf(out, "Active agents:   %v\n", s.ActiveAgents)
	} else {
		fmt.Fprintf(out, "Active agents:   (none)\n")
	}

	if len(s.ActiveWorkflows) > 0 {
		fmt.Fprintf(out, "Active workflows:%v\n", s.ActiveWorkflows)
	} else {
		fmt.Fprintf(out, "Active workflows:(none)\n")
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			fmt.Fprintf(out, "Lockfile status: missing (devrune.lock not found)\n")
		} else {
			fmt.Fprintf(out, "Lockfile status: error reading lockfile\n")
		}
		return nil
	}
	...
	if lockfileHash == s.LockHash {
		...
		fmt.Fprintf(out, "Status: stale (lockfile changed since last install)\n")
	}
