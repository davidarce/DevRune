...
// RenderCatalog writes the AGENTS.md guidance file for OpenCode.
// It injects captured workflow registry content into the catalog and appends
// MCP-derived Memory/Engram sections (matching Claude's catalog behavior).
func (r *OpenCodeRenderer) RenderCatalog(skills []model.ContentItem, rules []model.ContentItem, workflows []model.WorkflowManifest, destPath string) error {
	var sb strings.Builder

	sb.WriteString("# OpenCode Agent Catalog\n\n")
	...
	if len(workflows) > 0 {
		...
		for _, wf := range workflows {
			...
			// The full orchestrator instructions already exist in opencode.json as the
	...
	// Append MCP agent instructions (e.g. Engram memory protocol), matching Claude behavior.
	if memSection := generateMCPCatalogSections(r.normalizedMCPs); memSection != "" {
		sb.WriteString(memSection)
	}

	if err := os.MkdirAll(filepath.Dir(destPath), 0o755); err != nil {
		return fmt.Errorf("opencode: mkdir for catalog: %w", err)
	}
